---
title: "Internet-Related ACS Table Fetcher"
...
# We just want the internet-relevant variables - see <https://www.digitalinclusion.org/home-internet-maps/>
```{r}
internet_variables <-
...
## Pre-fetch the cartographic boundary shapefiles - see https://www.census.gov/programs-surveys/geography/technical-documentation/naming-convention/cartographic-boundary-file.html
```{r}
for (state in unique(fips_codes$state)) {
  if (state == "UM") next # "UM" doesn't have data
  print(paste("fetching cartographic boundary shapefiles for", state))
  if (state == "AL") { # first state - start accumulators
    ...
    county_cartographic_boundaries <-
      ...
      sf::st_transform(4326) %>%
      janitor::clean_names()
  } else { # not the first state - accumulate
...
## Save national boundary tables as GeoJSON
```{r}
sf::st_write(
...
## Drop columns we aren't using later
```{r}
...
## Main loop - collect the tables and write them as GeoJSON
```{r}
...
# for (itable in c("B28002")) {
  ...
  # Now we have "long" format data without geometry. We need wide format with geometry
